...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const bizcollab_app_email = settings['bizcollab_app_email'];
  const bizcollab_base_url = settings['bizcollab_base_url'];

  const url = checkAndCorrectURL(bizcollab_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | bizcollab',
    bizcollab_app_email,
