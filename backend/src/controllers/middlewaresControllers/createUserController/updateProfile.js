const mongoose = require('mongoose');

const updateProfile = async (userModel, req, res) => {
  const User = mongoose.model(userModel);

  const reqUserName = userModel.toLowerCase();
  const userProfile = req[reqUserName];

  if (userProfile.email === 'admin@admin.com') {
    return res.status(403).json({
      success: false,
      result: null,
      message: "you couldn't update demo informations",
    });
  }

  let updates = req.body.photo
    ? {
        email: req.body.email,
        name: req.body.name,
        mobile: req.body.mobile,
        companyName: req.body.companyName,
        gstNumber: req.body.gstNumber,
        photo: req.body.photo,
      }
    : {
        email: req.body.email,
        name: req.body.name,
        mobile: req.body.mobile,
        companyName: req.body.companyName,
        gstNumber: req.body.gstNumber,
      };
  
  // Find document by id and updates with the required fields
  const result = await User.findOneAndUpdate(
    { _id: userProfile._id, removed: false },
    { $set: updates },
    {
      new: true, // return the new result instead of the old one
    }
  ).exec();

  if (result && req.body.companyName) {
    const Setting = mongoose.model('Setting');
    await Setting.findOneAndUpdate(
      { settingKey: 'company_name', tenantId: result.tenantId },
      { $set: { settingValue: req.body.companyName, settingCategory: 'company_settings' } },
      { upsert: true }
    ).exec();
  }

  if (result && req.body.bizcollab_app_date_format) {
    const Setting = mongoose.model('Setting');
    await Setting.findOneAndUpdate(
      { settingKey: 'bizcollab_app_date_format', tenantId: result.tenantId },
      { $set: { settingValue: req.body.bizcollab_app_date_format, settingCategory: 'app_settings' } },
      { upsert: true }
    ).exec();
  }

  if (!result) {
    return res.status(404).json({
      success: false,
      result: null,
      message: 'No profile found by this id: ' + userProfile._id,
    });
  }
  
  return res.status(200).json({
    success: true,
    result: {
      _id: result?._id,
      enabled: result?.enabled,
      email: result?.email,
      name: result?.name,
      mobile: result?.mobile,
      companyName: result?.companyName,
      gstNumber: result?.gstNumber,
      photo: result?.photo,
      role: result?.role || 'owner',
    },
    message: 'we update this profile by this id: ' + userProfile._id,
  });
};

module.exports = updateProfile;
