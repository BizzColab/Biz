...
export default function ReadItem({ config }) {
  ...
  const itemsList = listState.map((item) => {
    return (
      <Row key={item.propsKey} className="read-item-row">
        <Col span={24}>
          <div className="read-item-label">{item.label}</div>
          <div className="read-item-value">{item.value}</div>
        </Col>
      </Row>
    );
  });
  ...
  return (
    <div style={show} className="read-item-card">
      {itemsList}
    </div>
  );
}
